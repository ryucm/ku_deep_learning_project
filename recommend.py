...
실행: python recommend.py --image path/to/your_clothes.jpg --model custom --top_k 5
"""
import torch
import numpy as np
import json
...
def cosine_similarity(query, index):
    """
    query: (512,)  — 입력 이미지 벡터
    index: (N, 512) — 데이터셋 전체 벡터
    반환: (N,) 유사도 점수 (-1 ~ 1, 높을수록 유사)
    """
...
def visualize(query_path, results):
    ...
    plt.show()
    print("recommendation_result.png 저장 완료")
